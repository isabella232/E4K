...
use server_agent_api::{create_workload_jwt, ApiVersion};
use std::borrow::Cow;
...
impl server::Route for Route {
    ...
    type PostBody = create_workload_jwt::Request;
    ...
    fn from_uri(
        ...
        if path != uri::CREATE_WORKLOAD_JTW {
            return None;
        }
        Some(Route {
    ...
    async fn post(self, body: Option<Self::PostBody>) -> server::RouteResponse {
        ...
        let res = self.api.create_workload_jwt(body).await;
        let res = match res {
            Ok(res) => res,
            Err(err) => {
